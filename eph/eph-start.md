...
```bash
kubectl run --restart=Never dbportforward -n ep-romank-investment --image=crbackbasesharedprd874.azurecr.io/external/socat/socat:1.7.4.4 --overrides='{"metadata":{"labels":{"sidecar.istio.io/inject":"false"},
"name":"dbportforward","namespace":"ep-romank-investment"},
"spec":{"containers":[{"args":["-d","tcp-listen:1433,fork,reuseaddr","'"tcp-connect:$(kubectl -n ep-romank-investment get configmaps database-ephemeral -o jsonpath="{.data.endpoint}"):1433"'"],
"image":"crbackbasesharedprd874.azurecr.io/external/socat/socat:1.7.4.4","name":"socat",
"securityContext":{"seccompProfile":{"type":"RuntimeDefault"},"allowPrivilegeEscalation":false,"readOnlyRootFilesystem": true,"capabilities":{"drop":["ALL"]}},
"resources":{"limits":{"cpu":"200m","memory":"256Mi"}}}],"restartPolicy":"Never"}}'
